  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`hits`)) {
        self$`hits` <- ApiClient$new()$deserializeObj(this_object$`hits`, "array[OrgSagebionetworksRepoModelSearchHit]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`facets`)) {
        self$`facets` <- ApiClient$new()$deserializeObj(this_object$`facets`, "array[OrgSagebionetworksRepoModelSearchFacet]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
