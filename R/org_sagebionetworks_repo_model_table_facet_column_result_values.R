...
OrgSagebionetworksRepoModelTableFacetColumnResultValues <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `columnName` = NULL, `facetType` = NULL, `jsonPath` = NULL, `facetValues` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.table.FacetColumnResultValues"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.table.FacetColumnResultValues\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.table.FacetColumnResultValues"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.table.FacetColumnResultValues\".", sep = ""))
        }
      ...
      if (!is.null(this_object$`facetValues`)) {
        self$`facetValues` <- ApiClient$new()$deserializeObj(this_object$`facetValues`, "array[OrgSagebionetworksRepoModelTableFacetColumnResultValueCount]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
...
',
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelTableFacetColumnResultValues: the required field `concreteType` is missing."))
      }
    },
