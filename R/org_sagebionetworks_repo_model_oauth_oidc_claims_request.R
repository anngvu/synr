  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`userinfo`)) {
        self$`userinfo` <- ApiClient$new()$deserializeObj(this_object$`userinfo`, "map(OrgSagebionetworksRepoModelOauthOIDCClaimsRequestDetails)", loadNamespace("synr"))
      }
      if (!is.null(this_object$`id_token`)) {
        self$`id_token` <- ApiClient$new()$deserializeObj(this_object$`id_token`, "map(OrgSagebionetworksRepoModelOauthOIDCClaimsRequestDetails)", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
