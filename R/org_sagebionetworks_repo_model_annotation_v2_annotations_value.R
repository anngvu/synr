...
#' @field value List of values as strings. Clients should use the \"type\" field to determine how to interpret the strings. Single values Annotations are represented as a list of size 1, and multiple values size > 1. Empty list or null is not allowed list(character)
  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`value`)) {
        self$`value` <- ApiClient$new()$deserializeObj(this_object$`value`, "array[character]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`value`)) {
        stopifnot(is.vector(input_json$`value`), length(input_json$`value`) != 0)
        tmp <- sapply(input_json$`value`, function(x) stopifnot(is.character(x)))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelAnnotationV2AnnotationsValue: the required field `value` is missing."))
      }
    },
