...
OrgSagebionetworksRepoModelRegistryEntityTypeMetadata <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`name`, `displayName` = NULL, `className` = NULL, `entityType` = NULL, `defaultParentPath` = NULL, `validParentTypes` = NULL, `aliases` = NULL, ...) {
      if (!missing(`name`)) {
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`validParentTypes`)) {
        self$`validParentTypes` <- ApiClient$new()$deserializeObj(this_object$`validParentTypes`, "array[character]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`aliases`)) {
        self$`aliases` <- ApiClient$new()$deserializeObj(this_object$`aliases`, "array[character]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelRegistryEntityTypeMetadata: the required field `name` is missing."))
      }
    },
