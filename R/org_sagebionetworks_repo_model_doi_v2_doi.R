  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`creators`)) {
        self$`creators` <- ApiClient$new()$deserializeObj(this_object$`creators`, "array[OrgSagebionetworksRepoModelDoiV2DoiCreator]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`titles`)) {
        self$`titles` <- ApiClient$new()$deserializeObj(this_object$`titles`, "array[OrgSagebionetworksRepoModelDoiV2DoiTitle]", loadNamespace("synr"))
      }
