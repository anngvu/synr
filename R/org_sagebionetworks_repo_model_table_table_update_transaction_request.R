...
OrgSagebionetworksRepoModelTableTableUpdateTransactionRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `entityId` = NULL, `changes` = NULL, `createSnapshot` = NULL, `snapshotOptions` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.table.TableUpdateTransactionRequest"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.table.TableUpdateTransactionRequest\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.table.TableUpdateTransactionRequest"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.table.TableUpdateTransactionRequest\".", sep = ""))
        }
      ...
      if (!is.null(this_object$`changes`)) {
        self$`changes` <- ApiClient$new()$deserializeObj(this_object$`changes`, "array[OrgSagebionetworksRepoModelTableTableUpdateRequest]", loadNamespace("synr"))
      }
...
',
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelTableTableUpdateTransactionRequest: the required field `concreteType` is missing."))
      }
    },
