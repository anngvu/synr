...
OrgSagebionetworksRepoModelDataaccessAccessApprovalSearchRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`accessorId`, `accessRequirementId` = NULL, `sort` = NULL, `nextPageToken` = NULL, ...) {
      if (!missing(`accessorId`)) {
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`sort`)) {
        self$`sort` <- ApiClient$new()$deserializeObj(this_object$`sort`, "array[OrgSagebionetworksRepoModelDataaccessAccessApprovalSearchSort]", loadNamespace("synr"))
      }
...
',
    ...
    validateJSON = function(input) {
      ...
      # check the required field `accessorId`
      if (!is.null(input_json$`accessorId`)) {
        if (!(is.character(input_json$`accessorId`) && length(input_json$`accessorId`) == 1)) {
          stop(paste("Error! Invalid data for `accessorId`. Must be a string:", input_json$`accessorId`))
        }
      } else {
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelDataaccessAccessApprovalSearchRequest: the required field `accessorId` is missing."))
      }
    },
    ...
    #' @export
    isValid = function() {
      # check if the required `accessorId` is null
      if (is.null(self$`accessorId`)) {
        return(FALSE)
      }
    ...
    getInvalidFields = function() {
      ...
      # check if the required `accessorId` is null
      if (is.null(self$`accessorId`)) {
        invalid_fields["accessorId"] <- "Non-nullable required field `accessorId` cannot be null."
      }
