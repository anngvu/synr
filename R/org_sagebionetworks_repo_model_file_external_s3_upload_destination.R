...
OrgSagebionetworksRepoModelFileExternalS3UploadDestination <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `storageLocationId` = NULL, `uploadType` = NULL, `banner` = NULL, `baseKey` = NULL, `stsEnabled` = NULL, `endpointUrl` = NULL, `bucket` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.file.ExternalS3UploadDestination"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.file.ExternalS3UploadDestination\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.file.ExternalS3UploadDestination"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.file.ExternalS3UploadDestination\".", sep = ""))
        }
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelFileExternalS3UploadDestination: the required field `concreteType` is missing."))
      }
    },
