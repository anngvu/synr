  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`enumValues`)) {
        self$`enumValues` <- ApiClient$new()$deserializeObj(this_object$`enumValues`, "array[character]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`jsonSubColumns`)) {
        self$`jsonSubColumns` <- ApiClient$new()$deserializeObj(this_object$`jsonSubColumns`, "array[OrgSagebionetworksRepoModelTableJsonSubColumnModel]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
