  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`relatedProjectIds`)) {
        self$`relatedProjectIds` <- ApiClient$new()$deserializeObj(this_object$`relatedProjectIds`, "array[character]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`reviewerIds`)) {
        self$`reviewerIds` <- ApiClient$new()$deserializeObj(this_object$`reviewerIds`, "array[character]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
