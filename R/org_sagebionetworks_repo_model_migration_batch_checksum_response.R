...
OrgSagebionetworksRepoModelMigrationBatchChecksumResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `migrationType` = NULL, `cheksums` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.migration.BatchChecksumResponse"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.migration.BatchChecksumResponse\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.migration.BatchChecksumResponse"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.migration.BatchChecksumResponse\".", sep = ""))
        }
      ...
      if (!is.null(this_object$`cheksums`)) {
        self$`cheksums` <- ApiClient$new()$deserializeObj(this_object$`cheksums`, "array[OrgSagebionetworksRepoModelMigrationRangeChecksum]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
...
',
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelMigrationBatchChecksumResponse: the required field `concreteType` is missing."))
      }
    },
