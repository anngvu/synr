...
OrgSagebionetworksRepoModelTableTableSchemaChangeRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `entityId` = NULL, `changes` = NULL, `orderedColumnIds` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.table.TableSchemaChangeRequest"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.table.TableSchemaChangeRequest\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.table.TableSchemaChangeRequest"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.table.TableSchemaChangeRequest\".", sep = ""))
        }
      ...
      if (!is.null(this_object$`changes`)) {
        self$`changes` <- ApiClient$new()$deserializeObj(this_object$`changes`, "array[OrgSagebionetworksRepoModelTableColumnChange]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`orderedColumnIds`)) {
        self$`orderedColumnIds` <- ApiClient$new()$deserializeObj(this_object$`orderedColumnIds`, "array[character]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
...
',
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelTableTableSchemaChangeRequest: the required field `concreteType` is missing."))
      }
    },
