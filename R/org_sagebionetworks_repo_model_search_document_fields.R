  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`acl`)) {
        self$`acl` <- ApiClient$new()$deserializeObj(this_object$`acl`, "array[character]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`update_acl`)) {
        self$`update_acl` <- ApiClient$new()$deserializeObj(this_object$`update_acl`, "array[character]", loadNamespace("synr"))
      }
