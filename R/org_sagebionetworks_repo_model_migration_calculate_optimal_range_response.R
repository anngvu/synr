...
OrgSagebionetworksRepoModelMigrationCalculateOptimalRangeResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `migrationType` = NULL, `ranges` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.migration.CalculateOptimalRangeResponse"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.migration.CalculateOptimalRangeResponse\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.migration.CalculateOptimalRangeResponse"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.migration.CalculateOptimalRangeResponse\".", sep = ""))
        }
      ...
      if (!is.null(this_object$`ranges`)) {
        self$`ranges` <- ApiClient$new()$deserializeObj(this_object$`ranges`, "array[OrgSagebionetworksRepoModelMigrationIdRange]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
...
',
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelMigrationCalculateOptimalRangeResponse: the required field `concreteType` is missing."))
      }
    },
