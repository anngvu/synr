...
#' @field keys Array of annotation keys list(character)
  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`keys`)) {
        self$`keys` <- ApiClient$new()$deserializeObj(this_object$`keys`, "array[character]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
    ...
    validateJSON = function(input) {
      ...
      # check the required field `keys`
      if (!is.null(input_json$`keys`)) {
        stopifnot(is.vector(input_json$`keys`), length(input_json$`keys`) != 0)
        tmp <- sapply(input_json$`keys`, function(x) stopifnot(is.character(x)))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelAnnotationV2Keys: the required field `keys` is missing."))
      }
    },
    ...
    #' @export
    isValid = function() {
      # check if the required `keys` is null
      if (is.null(self$`keys`)) {
        return(FALSE)
      }
    ...
    getInvalidFields = function() {
      ...
      # check if the required `keys` is null
      if (is.null(self$`keys`)) {
        invalid_fields["keys"] <- "Non-nullable required field `keys` cannot be null."
      }
