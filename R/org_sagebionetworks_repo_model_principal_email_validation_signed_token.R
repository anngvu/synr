...
OrgSagebionetworksRepoModelPrincipalEmailValidationSignedToken <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `hmac` = NULL, `version` = NULL, `expiresOn` = NULL, `createdOn` = NULL, `email` = NULL, `userId` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.principal.EmailValidationSignedToken"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.principal.EmailValidationSignedToken\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.principal.EmailValidationSignedToken"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.principal.EmailValidationSignedToken\".", sep = ""))
        }
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelPrincipalEmailValidationSignedToken: the required field `concreteType` is missing."))
      }
    },
