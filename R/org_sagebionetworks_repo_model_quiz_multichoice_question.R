...
OrgSagebionetworksRepoModelQuizMultichoiceQuestion <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `questionIndex` = NULL, `prompt` = NULL, `reference` = NULL, `docLink` = NULL, `helpText` = NULL, `exclusive` = NULL, `answers` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.quiz.MultichoiceQuestion"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.quiz.MultichoiceQuestion\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.quiz.MultichoiceQuestion"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.quiz.MultichoiceQuestion\".", sep = ""))
        }
      ...
      if (!is.null(this_object$`answers`)) {
        self$`answers` <- ApiClient$new()$deserializeObj(this_object$`answers`, "array[OrgSagebionetworksRepoModelQuizMultichoiceAnswer]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
...
',
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelQuizMultichoiceQuestion: the required field `concreteType` is missing."))
      }
    },
