...
OrgSagebionetworksRepoModelTableQueryResultBundle <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `queryResult` = NULL, `queryCount` = NULL, `selectColumns` = NULL, `maxRowsPerPage` = NULL, `columnModels` = NULL, `facets` = NULL, `sumFileSizes` = NULL, `lastUpdatedOn` = NULL, `combinedSql` = NULL, `actionsRequired` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.table.QueryResultBundle"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.table.QueryResultBundle\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.table.QueryResultBundle"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.table.QueryResultBundle\".", sep = ""))
        }
      ...
      if (!is.null(this_object$`selectColumns`)) {
        self$`selectColumns` <- ApiClient$new()$deserializeObj(this_object$`selectColumns`, "array[OrgSagebionetworksRepoModelTableSelectColumn]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`maxRowsPerPage`)) {
        self$`maxRowsPerPage` <- this_object$`maxRowsPerPage`
      }
      if (!is.null(this_object$`columnModels`)) {
        self$`columnModels` <- ApiClient$new()$deserializeObj(this_object$`columnModels`, "array[OrgSagebionetworksRepoModelTableColumnModel]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`facets`)) {
        self$`facets` <- ApiClient$new()$deserializeObj(this_object$`facets`, "array[OrgSagebionetworksRepoModelTableFacetColumnResult]", loadNamespace("synr"))
      }
      ...
      if (!is.null(this_object$`actionsRequired`)) {
        self$`actionsRequired` <- ApiClient$new()$deserializeObj(this_object$`actionsRequired`, "array[OrgSagebionetworksRepoModelDownloadActionRequiredCount]", loadNamespace("synr"))
      }
      self
    },
    #' To JSON string
...
',
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelTableQueryResultBundle: the required field `concreteType` is missing."))
      }
    },
