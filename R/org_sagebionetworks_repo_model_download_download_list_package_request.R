...
OrgSagebionetworksRepoModelDownloadDownloadListPackageRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `zipFileName` = NULL, `includeManifest` = NULL, `csvTableDescriptor` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.download.DownloadListPackageRequest"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.download.DownloadListPackageRequest\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.download.DownloadListPackageRequest"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.download.DownloadListPackageRequest\".", sep = ""))
        }
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelDownloadDownloadListPackageRequest: the required field `concreteType` is missing."))
      }
    },
