  ...
  public = list(
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`scopes`)) {
        self$`scopes` <- ApiClient$new()$deserializeObj(this_object$`scopes`, "array[character]", loadNamespace("synr"))
      }
      if (!is.null(this_object$`userInfoClaims`)) {
        self$`userInfoClaims` <- ApiClient$new()$deserializeObj(this_object$`userInfoClaims`, "map(OrgSagebionetworksRepoModelOauthOIDCClaimsRequestDetails)", loadNamespace("synr"))
      }
