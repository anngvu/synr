...
OrgSagebionetworksRepoModelFileBulkFileDownloadResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    initialize = function(`concreteType`, `resultZipFileHandleId` = NULL, `fileSummary` = NULL, `userId` = NULL, ...) {
      if (!missing(`concreteType`)) {
        if (!(`concreteType` %in% c("org.sagebionetworks.repo.model.file.BulkFileDownloadResponse"))) {
          stop(paste("Error! \"", `concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.file.BulkFileDownloadResponse\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`concreteType`)) {
        if (!is.null(this_object$`concreteType`) && !(this_object$`concreteType` %in% c("org.sagebionetworks.repo.model.file.BulkFileDownloadResponse"))) {
          stop(paste("Error! \"", this_object$`concreteType`, "\" cannot be assigned to `concreteType`. Must be \"org.sagebionetworks.repo.model.file.BulkFileDownloadResponse\".", sep = ""))
        }
      ...
      if (!is.null(this_object$`fileSummary`)) {
        self$`fileSummary` <- ApiClient$new()$deserializeObj(this_object$`fileSummary`, "array[OrgSagebionetworksRepoModelFileFileDownloadSummary]", loadNamespace("synr"))
      }
...
',
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`concreteType`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for OrgSagebionetworksRepoModelFileBulkFileDownloadResponse: the required field `concreteType` is missing."))
      }
    },
